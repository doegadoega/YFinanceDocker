...
echo "   curl \"$API_URL/tickerDetail?ticker=AAPL\""
echo "   curl \"$API_URL/tickerDetail?ticker=AAPL&period=1y\""
echo "   curl \"$API_URL/tickerDetail?ticker=7203.T&period=6mo\""
echo ""
echo "3. チャート生成:"
...
echo "curl \"$API_URL/tickerDetail?ticker=AAPL&period=1y\" | jq"
echo ""
echo "# 日本株の情報を取得"
echo "curl \"$API_URL/tickerDetail?ticker=7203.T\" | jq"
echo ""
echo "# チャートを画像で取得"
...
echo "=== API ドキュメント ==="
echo "現在のAPIエンドポイント:"
echo "・GET /search?q={query}&region={region} - 銘柄検索"
echo "・GET /tickerDetail?ticker={ticker}&period={period} - 包括的情報取得"
echo "・GET /chart?ticker={ticker}&period={period}&type={type} - チャート生成"
echo ""
echo "=== テスト用URL ==="
echo "検索: $API_URL/search?q=apple&region=US"
echo "情報: $API_URL/tickerDetail?ticker=AAPL&period=1mo"
echo "チャート: $API_URL/chart?ticker=AAPL&period=1mo&type=candle"
echo ""
echo "=== 注意事項 ==="
